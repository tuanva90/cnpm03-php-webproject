...
class Admin_Model_UserGroup extends Zend_Db_Table {
	...
	protected $_primary = 'user_group_id';
	
	public function itemInSelectbox($arrParam = null, $options = null) {
		$db = Zend_Registry::get ( 'connectDB' );
		if ($options == null) {
			$select = $db->select ()->from ( 'user_group', array ('user_group_id', 'name' ) );
	...
	public function countItem($arrParam = null, $options = null) {
		...
		$select = $db->select ()->from ( 'user_group AS ug', array ('COUNT(ug.user_group_id) AS totalItem' ) );
		
		if (! empty ( $filter ['filter_name'] )) {
			$filter_name = '%' . mysql_real_escape_string ( $filter ['filter_name'] ) . '%';
	...
	public function listItem($arrParam = null, $options = null) {
		...
		if ($options ['task'] == 'list') {
			$select = $db->select ()
						 ->from ( 'user_group AS ug', array ('user_group_id', 'name', 'status', 'sort_order' ) )
						 ->joinLeft ( 'user AS u', 'ug.user_group_id = u.user_group_id', 'COUNT(u.user_id) AS members' )
