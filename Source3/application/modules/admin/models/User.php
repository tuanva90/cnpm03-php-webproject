<?php
class Admin_Model_User extends Honey_Db_Table {
	...
	public function countItem($arrParam = null, $options = null) {
		...
		$select = $db->select ()->from ( $this->getPrefix() . 'user AS u', array ('COUNT(u.user_id) AS totalItem' ) );
	...
	public function listItem($arrParam = null, $options = null) {
		...
		if ($options ['task'] == 'list') {
			$select = $db->select ()
						 ->from ( $this->getPrefix() . 'user AS u', array ('user_id', 'username', 'status', 'email', 'date_added' ) )
						 ->joinLeft ( $this->getPrefix() . 'user_group AS ug', 'ug.user_group_id = u.user_group_id', 'name as group' );
	...
	public function getItem($arrParam = null, $options = null) {
		...
		if ($options ['task'] == 'info' || $options ['task'] == 'edit') {
			$db = Zend_Registry::get ( 'connectDB' );
			$select = $db->select ()->from ( $this->getPrefix() . 'user as u' )
									->joinLeft ( $this->getPrefix() . 'user_group as ug', 'u.user_group_id = ug.user_group_id', array ('name as group' ) )
	...
	public function checkExist($checkFor, $value) {
		...
		$select = $db->select ()->from ( $this->getPrefix() . 'user', array ('COUNT(user_id) AS totalItem' ) );
