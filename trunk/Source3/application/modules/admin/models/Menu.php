<?php
class Admin_Model_Menu extends Honey_Db_Table {
	...
	///////////////ThaoNX
	public function getItem2($menu_id, $options = null) {
		...
		if ($options ['task'] == 'info' || $options ['task'] == 'edit') {
			...
			$select = $db->select()
						 ...
						 ->where('md.language = ?', $options['lang'])
						 ->order('m.sort_order');
	...
	protected function getmenuDescriptions2($menu_id,$lang) {
		$db = Zend_Registry::get('connectDB');		
		$select = $db->select()
