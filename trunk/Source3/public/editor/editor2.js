function insertHTML(){
	document.getElementById('pos').value = document.getElementById('txtnoidung').innerHTML;
}
...
function Editor(instanceName, contain, defaultContent, width, height, str_directory){
  ...
  this.sm= function(el,s){
	...
	this.ltb=function(){
		...
		if(tt==1){
			s+='<tr height="25">';
			for(var i=0;i<this.bs.length;i++){
			if(this.bs[i][0] == "btnInsertImage")
			{
				s+='<td width="20">';
				s+='<span style="position:relative;" id="'+this.is+'_'+this.bs[i][0]+'_menu"></span>';
				s+= '<div id="'+this.is+'_'+this.bs[i][0]+'" style="background-image:url('+this.iconPath+this.bs[i][2]+');background-repeat:no-repeat;background-position: center center;" class="bOut" specialattr="yes" instanceName="'+this.is+'" title="'+this.bs[i][1]+'">';
	...
	this.display=function(id){
		...
		this.ltb();
		this.sa('tbeditor', 'mouseover', this.is + '.setHTMLto("txtnoidung")');
		//this.setHTMLto(id);
		/*if(this.showFootbar){
			for(var i=0;i<this.controlButtons.length;i++){
				var btnid=this.is+'_'+this.controlButtons[i][0];
				this.sa(btnid,'click', this.controlButtons[i][2]);
				this.sa(btnid,'mouseover',this.is+'.setStyle("'+btnid+'","mouseOn")');
				this.sa(btnid,'mouseout',this.is+'.setStyle("'+btnid+'","mouseOut")');
			}
		}*/
 ...
 **/
	var tEditorArr = new Array();
	var instanceNameArr = new Array();
	...
	function checkExistInstanceName(instanceName)
	{
		// kiem tra chuoi co rong ko
		if(instanceNameArr.length == 0)
			return false;

		var presentString = instanceNameArr.join('-');
		if(presentString.match(instanceName) == instanceName) return true;
		if(presentString.match(instanceName) != instanceName) return false;
	}
	function replace(id, id_replace, str_directory)
	{
		var instanceName;
		do{
			instanceName = randomString(3);
			if(!checkExistInstanceName(instanceName))
				break;
		}while(1);
		
		instanceNameArr.push(instanceName);
		RTE=new Editor(instanceName,id_replace,'',600, 350, str_directory);
		RTE.display(id);
...
$(function() {
	...
	$(document).ready(function(){
		...
		$('div [specialattr=yes]').live("click", function(){
			var instanceName = $(this).attr('instanceName');
			var count;
			for(count = 0; count < tEditorArr.length; count++)
			{
				if(tEditorArr[count].is == instanceName)
					currEditor = tEditorArr[count];
			}
			if(!currEditor)
			{
				$('<div id="insert-image-frm" title="Edit Page Header."><form><div id="tab-header"><ul id="tab-buttons"><li id="image-info" class="active-tab"><a href="javascript:void();">Thong tin hinh anh</a></li><li class="unactive-tab" id="insert-hyperlink"><a href="javascript:void();">Lien ket</a></li></ul></div> <div id="tab-content">'
					...
					buttons: {
						"Đồng ý": function() {
								...
								if(aLink){
									...
									if(!isIE){
										currEditor.fmt('inserthtml',aLink);
										}
									else{
										var ig=currEditor.ed.selection.createRange();
										ig.select();
										ig.pasteHTML(aLink);
										}
					     
									alert(currEditor.getHTML());
								$(this).dialog("close");
								}
						},
						"Bỏ qua": function() {
